    ...
    ErrorResult& aRv)
{
    ...
    for (const auto& url : aUrls) {
        ...
        nsresult rv = GetAbsoluteURL(url, baseUri, window->GetExtantDoc(), absoluteUrl);
    ...
    RefPtr<PresentationRequest> request = new PresentationRequest(window, std::move(urls));
    ...
    ErrorResult& aRv)
{
    ...
    nsCOMPtr<nsIPresentationService> service = do_GetService(PRESENTATION_SERVICE_CONTRACTID);
    ...
    nsCOMPtr<nsIPresentationServiceCallback> callback = new PresentationRequesterCallback(this, id, promise);
    nsCOMPtr<nsIPresentationTransportBuilderConstructor> constructor = PresentationTransportBuilderConstructor::Create();
    ...
    Promise* aPromise)
{
    ...
    RefPtr<PresentationConnection> connection = ControllerConnectionCollection::GetSingleton()->FindConnection(
        GetOwner()->WindowID(),
        aPresentationId,
    ...
    nsCOMPtr<nsIPresentationServiceCallback> callback = new PresentationReconnectCallback(this,
        aPresentationId,
        aPromise,
        connection);

    nsresult rv = service->ReconnectSession(mUrls,
        aPresentationId,
        nsIPresentationService::ROLE_CONTROLLER,
        callback);
    if (NS_WARN_IF(NS_FAILED(rv))) {
...
void PresentationRequest::FindOrCreatePresentationAvailability(RefPtr<Promise>& aPromise)
{
    MOZ_ASSERT(aPromise);
    ...
    RefPtr<PresentationAvailability> availability = collection->Find(GetOwner()->WindowID(), mUrls);
...
PresentationRequest::DispatchConnectionAvailableEvent(PresentationConnection* aConnection)
{
    ...
    RefPtr<PresentationConnectionAvailableEvent> event = PresentationConnectionAvailableEvent::Constructor(this,
    ...
    RefPtr<AsyncEventDispatcher> asyncDispatcher = new AsyncEventDispatcher(this, event);
...
bool PresentationRequest::IsProhibitMixedSecurityContexts(nsIDocument* aDocument)
{
...
bool PresentationRequest::IsPrioriAuthenticatedURL(const nsAString& aUrl)
{
    ...
    if (uriSpec.EqualsLiteral("about:blank") || uriSpec.EqualsLiteral("about:srcdoc")) {
        return true;
    }
